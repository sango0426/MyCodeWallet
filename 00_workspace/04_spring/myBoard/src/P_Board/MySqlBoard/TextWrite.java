...
/**
 * TextWrite 클래스 - 글 작성 기능을 담당하는 클래스입니다.
 */
public class TextWrite extends Db {

	/**
	 * 글 작성 메서드
	 *
	 * @return 새로 작성된 글의 인덱스 코드
	 */
	public static int TextWriteRun() {
		...
		textWrite: while (true) {
			...
			case "y":
				...
				Db.dbHub("Other", "Insert into board(textnumber, b_title, b_datetime, b_hit, b_text, b_user) value ("
						+ indexcode + ", '" + textTitle + "', now(), 0, '" + textBody + "', '" + memberid + "')");
