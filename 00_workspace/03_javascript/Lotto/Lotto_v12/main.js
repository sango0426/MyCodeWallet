...
- 보너스 번호도 화면에 출력하기
- 보너스 번호로 3등 당첨자 중 2등을 가려내기(이번 버전에 추가)
- 2등은 3등 당첨자만 대상으로 함(이번 버전에 추가)
- 5개를 맞추고 틀린 한 번호가 보너스 번호와 일치하면 2등임 <<< 2등 규칙(이번 버전에 추가)
- 2등이면 2등입니다.를 출력하기(이번 버전에 추가)
********************/
...
while(true) {
    comRanBonusNum = Math.floor(Math.random() * 45) + 1;
    if(comRanBonusNum != comRanNum[0] && comRanBonusNum != comRanNum[1] && comRanBonusNum != comRanNum[2] && comRanBonusNum != comRanNum[3] && comRanBonusNum != comRanNum[4] && comRanBonusNum != comRanNum[5]) {
        document.write("컴퓨터의 보너스 번호 : " + comRanBonusNum);
        document.write("<br>");
        break;
    }
}
...
switch(win) {
    ...
    case 5:
        ...
        //2등 처리
        for(var i = 0; i < 6; i++) { // 보너스 번호가 3등 당첨 유저의 나머지 한번호와 일치하는경우 2등 처리
            if(comRanBonusNum == LottoNum[i]) {
                str = "축하합니다! 2등입니다!";
            }
        }
        break;
